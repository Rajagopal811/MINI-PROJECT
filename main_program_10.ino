...
void loop() {
  ...
  if (keyi=='A'){
    ...
    Serial.println("full tnk . . .");
    Serial.println(full_mnt);
    inpamnt=full_mnt;
    } 
  else{
...
long full_tank(long amnt)                          //. . FULL . . .. . TANK . . .
 {
  ...
  Serial.println("ultrsonic");
  // . .. 
  for(; ;){
    if ((distance>=4) && (curnt_mnt < amnt)){
      digitalWrite(rely, LOW);
      delay(500);                        //30ml . . per . second. . .= approx 120ml. . in real
      digitalWrite(rely, HIGH);
      curnt_mnt=curnt_mnt+(0.120*100);              // 12rs . . .for  . .. 120ml
      Serial.println(curnt_mnt);
      delay(200);
      //
      digitalWrite(trigPin, LOW);         // Calculating updated distance . . .. 
      delayMicroseconds(2);
      //
